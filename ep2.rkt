...
;                NT: ("A" "B")
(define G (list (list "S" (list "a" "b" "A")) (list "A" (list "a" "A")) (list "A" (list "c" "A")) ) )
(define NT (list "S" "A"))
...
(define (achaTodasFraseProfundidadeN SI G NT novafrase SIZE )
  (if (empty? SI)
      novafrase
      (achaTodasFraseProfundidadeN (frasesDeTamanhoNOuMenor (substituiNTFrase3 SI (procuraRegrasConjuntoNT (procuraNT2 SI NT (list)) G (list)) (list)) SIZE (list)) G NT (appendToListIfNotPresent SI novafrase) SIZE )
  )
)
